...
from agent_watermark.experiments.evaluate_watermark import existing_manifest
...
def author_scaling(
    ...
    for size in sizes:
        decoys = [f"decoy-lab-{i:02d}" for i in range(max(0, size - len(base)))]
        candidates = base + decoys
...
def plot_paper_results(
    ...
    ax.set_title("Author-set scalability")
    ax.grid(alpha=0.25)
    ax.legend()
    fig.tight_layout()
    fig.savefig(plot_dir / "paper_author_scaling.png", dpi=180)
    plt.close(fig)
...
def main() -> None:
    ...
    manifest = load_manifest(args.evaluation_dir, args.logs)
    ...
    features = feature_table(args.evaluation_dir, manifest)
    ...
    granularity_df = timestamp_granularity_study(manifest, args.authors, args.min_margin, args.min_confidence)
    ...
    scaling_df.to_csv(args.out / "paper_author_scaling.csv", index=False)
    ...
    plot_paper_results(baseline_df, scaling_df, granularity_df, stealth_df, detectability_df, args.out)
    write_summary(baseline_df, scaling_df, granularity_df, stealth_df, detectability_df, args.out)
